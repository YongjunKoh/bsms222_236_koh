...
All datasets are tidy except a.
...
```
Notice that here we used `tota`l and `population` inside the function, which are objects that are **not** defined in our workspace. But why don’t we get an error?
...
Q1. Load the **dplyr** pacakge and the murders dataset.
```{r}
library(dplyr)
library(dslabs)
data(murders)
