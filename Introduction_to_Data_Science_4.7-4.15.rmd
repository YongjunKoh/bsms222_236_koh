...
Q1. We will provide some basic facts about blood pressure. First let’s select a group to set the standard. We will use 20-to-29-year-old females. `AgeDecade` is a categorical variable with these ages. Note that the category is coded like " 20-29", with a space in front! What is the average and standard deviation of systolic blood pressure as saved in the `BPSysAve` variable? Save it to a variable called `ref`.
...
ref
```
Q2. Using a pipe, assign the average to a numeric variable `ref_avg`. Hint: Use the code similar to above and then `pull`.
```{r}
ref <- NHANES %>% 
  filter(Gender == "female" & AgeDecade == " 20-29") %>%
  summarize(average = mean(BPSysAve, na.rm = TRUE), standard_deviation = sd(BPSysAve, na.rm = TRUE)) %>%
  pull(average) -> ref_avg
ref_avg
```
Q3. Now report the min and max values for the same group.
```{r}
NHANES %>% 
  filter(Gender == "female" & AgeDecade == " 20-29") %>%
  summarize(minimum = min(BPSysAve, na.rm = TRUE), maximum = max(BPSysAve, na.rm = TRUE))
```
Q4. Compute the average and standard deviation for females, but for each age group separately rather than a selected decade as in question 1. Note that the age groups are defined by `AgeDecade`. Hint: rather than filtering by age and gender, filter by `Gender` and then use `group_by`.
```{r}
NHANES %>%
  ...
  summarize(average = mean(BPSysAve, na.rm = TRUE), standard_deviation = sd(BPSysAve, na.rm = TRUE))
```
Q5. Repeat exercise 4 for males.
```{r}
NHANES %>%
  filter(Gender == "male") %>%
  group_by(AgeDecade) %>%
  summarize(average = mean(BPSysAve, na.rm = TRUE), standard_deviation = sd(BPSysAve, na.rm = TRUE))
```
Q6. We can actually combine both summaries for exercises 4 and 5 into one line of code. This is because `group_by` permits us to group by more than one variable. Obtain one big summary table using `group_by(AgeDecade, Gender)`.
```{r}
NHANES %>%
  group_by(AgeDecade, Gender) %>%
  summarize(average = mean(BPSysAve, na.rm = TRUE), standard_deviation = sd(BPSysAve, na.rm = TRUE))
```
Q7. For males between the ages of 40-49, compare systolic blood pressure across race as reported in the `Race1` variable. Order the resulting table from lowest to highest average systolic blood pressure.
```{r}
NHANES %>%
...
Q1. Load the `murders` dataset. Which of the following is true?
...
Q2. Use `as_tibble` to convert the `murders` data table into a tibble and save it in an object called `murders_tibble`.
```{r}
murders_tibble <- as_tibble(murders)
class(murders_tibble)
```

Q3. Use the `group_by` function to convert `murders` into a tibble that is grouped by region.
```{r}
murders %>% group_by(region) %>% class()
```

Q4. Write tidyverse code that is equivalent to this code:
```{r}
...
```
Q5. Use the `map_df` to create a data frame with three columns named `n`, `s_n`, and `s_n_2`. The first column should contain the numbers 1 through 100. The second and third columns should each contain the sum of 1 through $n$ with $n$ the row number.
```{r}
compute_s_n_2 <- function(x){
  tibble(n = x, s_n = sum(1:x), s_n_2 = sum(1:x))
}
map_df(1:100, compute_s_n_2)
```
or
```{r}
compute_s_n_3 <- function(n){
  x <- 1:n
  tibble(n = n, s_n = sum(x), s_n_2 = sum(x))
}
map_df(1:100, compute_s_n_3)
```
