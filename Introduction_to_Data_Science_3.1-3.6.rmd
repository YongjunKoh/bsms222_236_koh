...
For more on this more advanced topic we recommend the R packages book[^namespace].
...
```
In each iteration $n=1$, $n=2$, etc..., we compute $S_{n}$ and store it in the $n$th entry of `s_n`.
...
Q1. What will this conditional expression return?
```{r}
x <- c(1,2,-3,4)
...
sum_n(5000)
identical(sum(1:5000), sum_n(5000))
```
...
a = read.delim("rna_seq_example.txt")
altman_plot(log2(a$WT), log2(a$KO))
```
...
Q7. Write a function `compute_s_n` that for any given $n$ computes the sum $S_{n} = 1^2 + 2^2 + 3^2 + ... + n^2$. Report the value of the sum when $n = 10$.
```{r}
compute_s_n <- function(n){
  x <- 1:n
  sum(x^2)
...
compute_s_n(10)
```
Q8. Define an empty numerical vector `s_n` of size 25 using `s_n <- vector("numeric", 25) and store in the results of $S_{1}, S_{2}, ... , S_{25}$ using a for-loop.
```{r}
...
```
Q10. 10. Repeat exercise 8, but this time use `map_dbl`.
```{r}
...
```
Q11. 11. Plot $S_{n}$ versus $n$. Use points defined by $n = 1,...,25$. 
```{r}
n <- 1:25
Sn <- sapply(x, compute_s_n)
plot(n, Sn)
```
Q12. Confirm that the formula for this sum is $S_{n} = n(n+1)(2n+1)/6$.
```{r}
n <- 1:25
identical(Sn, n*(n+1)*(2*n+1)/6)
```