...
![](https://rafalab.github.io/dsbook/book_files/figure-html/state-region-distribution-1.png){width=50%}
...
![](https://rafalab.github.io/dsbook/book_files/figure-html/ecdf-1.png){width=50%}
...
![](https://rafalab.github.io/dsbook/book_files/figure-html/height-histogram-1.png){width=50%}
...
![](https://rafalab.github.io/dsbook/book_files/figure-html/example-of-smoothed-density-1.png){width=50%}
...
![](https://rafalab.github.io/dsbook/book_files/figure-html/simulated-data-histogram-1-1.png){width=50%}

The smaller we make the bins, the smoother the histogram gets. Here are the histograms with bin width of 1, 0.5, and 0.1:

![](https://rafalab.github.io/dsbook/book_files/figure-html/simulated-data-histogram-2-1.png){width=50%}
...
![](https://rafalab.github.io/dsbook/book_files/figure-html/simulated-density-1-1.png){width=50%}
...
![](https://rafalab.github.io/dsbook/book_files/figure-html/smooth-density-2-1.png){width=50%}
...
![](https://rafalab.github.io/dsbook/book_files/figure-html/densities-different-smoothness-1.png){width=50%}
...
![](https://rafalab.github.io/dsbook/book_files/figure-html/area-under-curve-1.png){width=50%}
...
![](https://rafalab.github.io/dsbook/book_files/figure-html/example-of-smoothed-density-2-1.png){width=50%}
...
![](https://rafalab.github.io/dsbook/book_files/figure-html/two-densities-one-plot-1.png){width=50%}
...
Q1. In the `murders` dataset, the region is a categorical variable and the following is its distribution:

```{r}
#> `summarise()` ungrouping output (override with `.groups` argument)
```

![](https://rafalab.github.io/dsbook/book_files/figure-html/barplot-exercise-1.png){width=50%}
...
Q3. The plot below shows the eCDF for male heights:

![](https://rafalab.github.io/dsbook/book_files/figure-html/ecdf-exercise-1.png){width=50%}
...
The answer is c, 69 inches.

Q5. Here is an eCDF of the murder rates across states:

![](https://rafalab.github.io/dsbook/book_files/figure-html/ecdf-exercise-2-1.png){width=50%}
...
Q7. Below is a histogram of male heights in our `heights` dataset:

![](https://rafalab.github.io/dsbook/book_files/figure-html/height-histogram-exercise-1.png){width=50%}
...
All a-d are wrong. The answer is 47. 
```{r}
heights %>% filter(sex == "Male" & between(height, 63.5, 65.5) == TRUE) %>% count()
```
...
```{r}
heights %>% filter(sex == "Male" & height < 60) %>% count() -> n
heights %>% filter(sex == "Male") %>% count() -> t
n/t * 100
```
The answer is a, about 1%.

Q9. Based on the density plot below, about what proportion of US states have populations larger than 10 million?

![](https://rafalab.github.io/dsbook/book_files/figure-html/density-exercise-1.png){width=50%}
...
The answer is b, 0.15.

Q10. Below are three density plots. Is it possible that they are from the same dataset?

![](https://rafalab.github.io/dsbook/book_files/figure-html/density-exercise-2-1.png){width=50%}
...
![](https://rafalab.github.io/dsbook/book_files/figure-html/normal-distribution-density-1.png){width=50%}
...
![](https://rafalab.github.io/dsbook/book_files/figure-html/data-and-normal-densities-1.png){width=50%}
...
First look back at the formula for the normal distribution and note that what is being exponentiated is $-z^2/2$ with $z$ equivalent to $s$ in standard units. Because the maximum of $e^{-z^2/2}$ is when $z=0$, this explains why the maximum of the distribution occurs at the average. It also explains the symmetry since $-z^2/2$ is symmetric around 0. Second, note that if we convert the normally distributed data to standard units, we can quickly know if, for example, a person is about average ($z=0$), one of the largest ($z \approx 2$), one of the smallest ($z \approx -2$), or an extremely rare occurrence ($z>3$ or $z<-3$). Remember that it does not matter what the original units are, these rules apply to any data that is approximately normal.
...
First let’s define the theoretical quantiles for the normal distribution. In statistics books we use the symbol $\Phi(x)$ to define the function that gives us the probability of a standard normal distribution being smaller than $x$. So, for example, $\Phi(-1.96) = 0.025$ and $\Phi(1.96) = 0.975$. In R, we can evaluate $\Phi(x)$ using the `pnorm` function.

```{r}
pnorm(-1.96)
```
The inverse function $\Phi^{-1}(x)$ gives us the *theoretical quantiles* for the normal distribution. So, for example, $\Phi^{-1}(0.975) = 1.96$. In R, we can evaluate the inverse of $\Phi(x)$ using the `qnorm` function.
...
![](https://rafalab.github.io/dsbook/book_files/figure-html/hist-qqplot-non-normal-data-1.png){width=50%}
...
![](https://rafalab.github.io/dsbook/book_files/figure-html/first-boxplot-1.png){width=50%}
