...
vector1
```
Q8. Create a vector of numbers that starts at 6, does not pass 55, and adds numbers in increments of 4/7: 6, 6 + 4/7, 6 + 8/7, and so on. How many numbers does the list have? Hint: use `seq` and `length`.
```{r}
length(seq(6, 55, 4/7))
```
...
class(a<-1L)
```
Q12. Define the following vector:
```{r}
x <- c("1", "3", "5")
```
...
pop[1]
```
Q2. Now instead of the smallest population size, find the index of the entry with the smallest population size. Hint: use `order` instead of `sort`.
```{r}
...
```
Q5. You can create a data frame using the `data.frame` function. Here is a quick example:
```{r}
temp <- c(35, 88, 42, 84, 81, 30)
city <- c("Beijing", "Lagos", "Paris", "Rio de Janeiro", "San Juan", "Toronto")
...
sum(ind)
```
The `na_example` vector has 145 `NA`s.
...
$$\left[\begin{array}{rrr}a\\b\\c\\d\end{array}\right] + \left[\begin{array}{rrr}e\\f\\g\\h\end{array}\right] = \left[\begin{array}{rrr}a+e\\b+f\\c+g\\d+h\end{array}\right]$$
...
```{r}
f_temp <- c(35, 88, 42, 84, 81, 30)
city <- c("Beijing", "Lagos", "Paris", "Rio de Janeiro", "San Juan", "Toronto")
c_temp <- (5/9)*(f_temp - 32)
city_temps <- data.frame(name = city, Fahrenheit = f_temp, Celcius = c_temp)
city_temps
```
Q2. What is the following sum $1 + 1/2^2 + 1/3^2 + ... 1/100^2$? Hint: thanks to Euler, we know it should be close to $\pi^2/6$.
```{r}
euler <- 1/(1:100)^2
sum(euler)
pi^2/6
...
low
```
Q2. Now use the results from the previous exercise and the function `which` to determine the indexes of `murder_rate` associated with values lower than 1.
```{r}
ind <- which(low)
ind
```
...
murders$state[ind]
```
Q4. Now extend the code from exercises 2 and 3 to report the states in the Northeast with murder rates lower than 1. Hint: use the previously defined logical vector `low` and the logical operator `&`.
```{r}
...
Q1. We made a plot of total murders versus population and noted a strong relationship. Not surprisingly, states with larger populations had more murders.
```{r}
library(dslabs)
data(murders)
...
```{r}
log10_population_in_millions <- log10(population_in_millions)
