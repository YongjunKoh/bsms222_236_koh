...
![](https://rafalab.github.io/dsbook/R/img/csv-file.png)
...
```{r}
file.copy(fullpath, "murders.csv")
file.copy("/cloud/project/murders.csv", "murders2.csv")
```
...
Q1. Use the `read_csv` function to read each of the files that the following code saves in the `files` object:
...
Q2. Note that the last one, the `olive` file, gives us a warning. This is because the first line of the file is missing the header for the first column.
...
```{r}
dat <- read_csv(fullpath[7], col_names = F, skip = 1)
dat
```

Q3. A problem with the previous approach is that we don’t know what the columns represent. Type:
```{r}
names(dat)
```
...
Q1. Pick a measurement you can take on a regular basis. For example, your daily weight or how long it takes you to run 5 miles. Keep a spreadsheet that includes the date, the hour, the measurement, and any other informative variable you think is worth keeping. Do this for 2 weeks. Then make a plot.


```{r}
distance_walk <- read.csv("apple_health_export.txt", sep="\t")
distance_walk %>% 
  mutate(full_date = as.Date(paste(as.character(year), as.character(month), as.character(date), sep = "-"))) -> distance_walk
p <- ggplot(distance_walk, aes(x=full_date, y=km)) +
  geom_line() + 
  scale_x_date(date_breaks = "4 month", date_labels = "%b-%y") +
  xlab("Date") +
  ylab("Distance (km)") +
  ggtitle("Daily walking distance")
p
```
Wrong data at Oct-18...?